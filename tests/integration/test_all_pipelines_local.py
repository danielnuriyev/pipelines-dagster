"""
Comprehensive test for all test_ pipelines on local Dagster.

This test:
1. Deploys all pipeline code to local Dagster
...
def deploy_to_local_dagster():
    """Deploy code to local Dagster by reloading the workspace."""
    print("\n=== Deploying to Local Dagster ===")

    # For local development, just reload the workspace
    # Assuming Dagster dev server is already running
    reload_workspace()
    print("Local deployment complete!")


@pytest.fixture(scope="module")
def deployed_local_dagster():
    """Deploy code to local Dagster once for the test module."""
    deploy_to_local_dagster()
    yield
    # No cleanup needed


@pytest.fixture
def test_setup(deployed_local_dagster):
...
def test_all_pipelines_local(test_setup):
    """
    Test all test_ pipelines on local Dagster:
    1. Discover all test_ pipeline assets
    2. Materialize them one by one
    3. Verify all materializations succeeded
    """
    print("\n" + "=" * 80)
    print("TEST ALL PIPELINES - LOCAL DAGSTER")
    print("=" * 80)
