#!/usr/bin/env python3
"""
Create a CSV file on S3 with 10 records matching the lakehouse.test.test_d schema.
...
def create_test_csv():
    """Create and upload a CSV file with test data to S3."""
    ...
    s3_bucket = "warehouse"
    s3_key = "test_data/test_d_sample.csv"
    s3_access_key = "admin"
    ...
    print(f"Successfully uploaded CSV file to s3://{s3_bucket}/{s3_key}")
    print(f"File size: {len(csv_content.encode('utf-8'))} bytes")
