...
def create_op_for_step(step_name: str, executor_func: Callable, step_config: dict, job_name: str):
    ...
    Note: These ops are created for metadata/future use. Currently execution
    uses the recursive engine which calls executor functions directly.
    """
...
def make_graph_asset_from_steps(
    ...
    Uses a recursive execution engine that supports:
    - Single-step pipelines
    - Multi-step pipelines
    - Batching at any step
    - Nested batching (multiple steps with batch_size)
    
    Note: Individual steps appear in logs but not as separate ops in UI.
    This is a limitation of Dagster's dynamic output system which doesn't
    support nested fan-out (multiple DynamicOut ops in sequence).
    """

    if not steps:
        raise ValueError(f"No steps defined for job: {job_name}")

    # Create ops for each step (for metadata/reference)
    step_ops_list = []
    ...
    # Create asset with recursive execution engine
    @asset(
        key=asset_key,
